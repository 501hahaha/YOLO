...
Stage 1: 训练 (顺序, 单 GPU)
    │
Stage 2: 验证 (并行 — 多实验时可委托 N 个 Codex 子任务)
    │
...
- 若存在多个实验（`runs/train/*/weights/best.pt`） → **并行验证**:
  1. 扫描所有 `best.pt`
  2. 为每个模型创建一个独立的 Codex 子任务，并将 `../../references/yolo-validator.md` 作为提示模板
  3. 收集结果，按 mAP@0.5 排名
  4. 输出对比表格

参考: `../yolo-validate/SKILL.md`
...
**多格式并行**（可选）: 若用户指定多格式，可委托 N 个 Codex 子任务并行导出；提示模板见 `../../references/yolo-exporter.md`。
...
2. **并行双重验证**:
   - 创建独立子任务运行 `test_det_onnx.py`
   - 创建独立子任务运行 `test_det_kmodel.py`
   - 两者独立运行，对比检测结果
3. 报告余弦相似度：
   - \> 0.98: ✅ 精度损失极小
