---
name: yolo-exporter
description: Exports a YOLOv5 PyTorch model to a single target format (ONNX, TensorRT, TorchScript, TFLite, etc.). Designed as a Codex subtask prompt template for parallel multi-format export.
---

# YOLOv5 Model Exporter (Codex Sub-Agent Template)

导出**一个**模型到**一个**格式。N 个格式就创建 N 个 Codex 子任务。
